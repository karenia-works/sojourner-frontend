...
async function getOrder(
    ...
    let tttOrder = await Axios.get(
        config.backend.address + config.backend.orderEndpoint(id)
    )
...
async function findOrderByRoom(
    id: string
): Promise<Order[]> {
    ...
    if (Orders.status == 200) {
        ...
        throw new Error('findByRoomError')
}

async function findOrderByUser(
    id: string
): Promise<Order[]> {
    let Orders = await Axios.get<Order[]>(
        config.backend.address + config.backend.orderByUser(id)
    )
    if (Orders.status == 200) {
        return Orders.data
    }
